...
![](images/buffer-overflows/overflow-stack-1.png){fig-alt="a diagram of the stack before the buffer overflow"}
:::
...
## Stack for vulnerable()
::: {.right-float-img .noboxshadow}
![](images/buffer-overflows/overflow-stack-2.png){fig-alt="a diagram of the stack part-way through the buffer overflow"}
:::
...
## Stack for vulnerable()
::: {.right-float-img .noboxshadow}
![](images/buffer-overflows/overflow-stack-3.png){fig-alt="a diagram of the stack after the buffer overflow"}
:::
...
## Stack for vulnerable()
::: {.right-float-img .noboxshadow}
![](images/buffer-overflows/return-to-stack.png){fig-alt="a diagram showing a return address to a stack location"}
:::
...
The previous code had a .data section, which we can remove:
...
afterString:  
    ...
    ret
```
This can be compiled the same way as the previous example.
...
- Note we are back to using the .data section
...
- To remove these values we:
  - Use smaller register 'parts' to eliminate extra 0x00's
