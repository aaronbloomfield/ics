...
![](images/encryption/tcpdump.png){fig-alt="an image of a TCP dump"}
...
## Cipher Taxonomy
![](images/encryption/cipher-taxonomy.png){fig-alt="cipher taxonomy"}
...
## Caesar Cipher
![](images/encryption/caesar-cipher.png){fig-alt="an image of a Caesar cipher"}
...
- Cracking
  ...
  - Example: crack "s psxn iyeb vkmu yp pksdr nscdeblsxq" online [here](http://md5decrypt.net/en/Caesar/) (select 'brute force')
...
![](images/encryption/vigenere-cipher.png){fig-alt="an image of a Vigenere cipher table"}
...
## Vigenere cipher
![](images/encryption/vigenere-cipher.png){fig-alt="vigenere table"}
...
## Rotor machines
![](images/encryption/rotor-machine.png){fig-alt="an image of a rotor machine"}
...
## Enigma machine
![](images/encryption/enigma-machine.png){fig-alt="a picture of the enigma machine"}
...
## Key Round Schedule
[![](https://upload.wikimedia.org/wikipedia/commons/b/be/AES-Key_Schedule_128-bit_key.svg){fig-alt="AES key round images"}{style="width:500px;float:right;margin:10px;border:10px solid white"}](https://commons.wikimedia.org/wiki/File:AES-Key_Schedule_128-bit_key.svg)
...
## AES: ShiftRows step
[![](https://upload.wikimedia.org/wikipedia/commons/6/66/AES-ShiftRows.svg){fig-alt="AES key round images: shiftrows step"}](https://commons.wikimedia.org/wiki/File:AES-ShiftRows.svg)
...
## AES: MixColumns step
[![](https://upload.wikimedia.org/wikipedia/commons/7/76/AES-MixColumns.svg){fig-alt="AES key round images: mixcolumns step"}](https://commons.wikimedia.org/wiki/File:AES-MixColumns.svg)
...
[![](https://upload.wikimedia.org/wikipedia/commons/a/ad/AES-AddRoundKey.svg){fig-alt="AES key round images: addroundkey step"}{style="width:600px;float:right;margin:10px;border:10px solid white"}](https://commons.wikimedia.org/wiki/File:AES-AddRoundKey.svg)
...
## Why does this work?
- Fermat's little theorem: $a^p-a \mod p=0$, but only if $p$ is prime
  - Equivalent phrasing: $a^p \equiv a (\text{mod } p)$
  - Equivalent phrasing: $a^{p-1} \equiv 1 (\text{mod } p)$
- If $p$ is not prime, then that equality will not hold if $a$ and $p$ share factors
  - Consider $a=2$ and $p=6$
    - $2^{6-1} \equiv 1 (\text{mod } p)$ yields $32 \equiv 1 (\text{mod } p)$, but $32 \text{ mod } 6=2$
- But how and why does Fermat's little theorem work?
  - There is an entire [Wikipedia page](https://en.wikipedia.org/wiki/Proofs_of_Fermat%27s_little_theorem) on the proofs of it


<!-- .slide: class="no-border" -->
## Google's recruitment campaign
![](https://statisticallyinsignificant.files.wordpress.com/2011/05/google.jpg){fig-alt="Google recruitment billboard"}
...
![](images/encryption/license-plate.jpg){fig-alt="RSA license plate"}
...
- I modify the ASCII codes for this example
 - By subtracting 30 from each value
...
- If multiple receivers share the same $e$, but different $p$, $q$, and $n$, then the same clear text message encrypted for the multiple receivers can be cracked via the Chinese remainder theorem
...
## Security
::: {.stretch}
![](https://imgs.xkcd.com/comics/security.png){fig-alt="xkcd Security comic"}{title="Actual actual reality: nobody cares about his secrets.  (Also, I would be hard-pressed to find that wrench for $5.)" style='border:10px solid white'}  
:::
::: {.r-stack}
...
![](images/encryption/secp256k1-a.svg){fig-alt="an elliptic curve"}
...
## Random Number
::: {.stretch}
![](https://imgs.xkcd.com/comics/random_number.png){fig-alt="xkcd on random numbers"}{title="RFC 1149.5 specifies 4 as the standard IEEE-vetted random number." style='border:10px solid white'}  
:::
::: {.r-stack}
...
![](http://blog.cloudflare.com/content/images/2017/11/lava-lamps.jpg){fig-alt="lava lamps at cloudflare's HQ"}
...
- Different Linux distributions will package them for easy installation
...
## Security Holes
::: {.stretch}
![](https://imgs.xkcd.com/comics/security_holes.png){fig-alt="xkcd on security holes"}{title="True story: I had to try several times to upload this comic because my ssh key was blacklisted." style='border:10px solid white'  }
:::
::: {.r-stack}
...
## Dependency
::: {.stretch}
![](https://imgs.xkcd.com/comics/dependency.png){fig-alt="xkcd on dependencies"}{title="Someday ImageMagick will finally break for good and we&#39;ll have a long period of scrambling as we try to reassemble civilization from the rubble." style='border:10px solid white' srcset="https://imgs.xkcd.com/comics/dependency_2x.png 2x" }
:::
::: {.r-stack}
...
![](images/encryption/dagapeyeff-cipher.png){fig-alt="an image of the Dagapeyeff cipher"}
:::
...
## 9. Kryptos
::: {.right-float-img .img-size-600}
![](https://listverse.com/wp-content/uploads/2007/10/kryptos-tm.jpg){fig-alt="an image of Kryptos"}
:::
...
::: {.right-float-img .img-size-600}
![](http://listverse.com/wp-content/uploads/2007/10/shug4big-tm.jpg){fig-alt="an image of the Shugborough Hall Inscription"}
:::
...
::: {.right-float-img .img-size-600}
![](http://listverse.com/wp-content/uploads/2007/10/10.1-tm.jpg){fig-alt="an image of the Chinese Gold Bar Cipher"}
:::
...
## 6. Chaocipher
::: {.right-float-img .img-size-600}
![](http://listverse.com/wp-content/uploads/2007/10/jfb-tm.jpg){fig-alt="an image of the Chaocipher"}
:::
...
::: {.right-float-img .img-size-600}
![](http://listverse.com/wp-content/uploads/2007/10/elgciph-tm.jpg){fig-alt="an image of the Dorabella Cipher"}
:::
...
## 4. Beale Ciphers
::: {.right-float-img .img-size-600}
![](http://listverse.com/wp-content/uploads/2007/10/700px-beale-1.svg-tm.jpg){fig-alt="an image of the Beale Ciphers"}
:::
...
## 3. Linear A
::: {.right-float-img .img-size-600}
![](http://listverse.com/wp-content/uploads/2007/10/lina-tm.jpg){fig-alt="an image of Linear A"}
:::
...
::: {.right-float-img .img-size-600}
![](http://listverse.com/wp-content/uploads/2007/10/awr-6vm2-tm.jpg){fig-alt="an image of the Voynich Manuscript"}
:::
- An entire book in a secret script that is at least 400 years old
  - 272 pages, although about 30 are missing
- Lists unidentified plants, as well as herbal recopies, astrological diagrams
...
![](http://listverse.com/wp-content/uploads/2007/10/phaistos-tm.jpg){fig-alt="an image of the Phaistos Disk"}
:::
