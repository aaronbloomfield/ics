...
::: {.noboxshadow}
![](images/cryptocurrency/aaron-dollar.png)
:::
...
- Some examples:
  ...
  - ![](https://upload.wikimedia.org/wikipedia/commons/e/e3/Moneda_1_Bol%C3%ADvar_2021.png) Due to the [crisis in Venezuela](https://en.wikipedia.org/wiki/Crisis_in_Venezuela), the Venezuelan [bolivar soberano](https://en.wikipedia.org/wiki/Venezuelan_bol%C3%ADvar) (VEB), and its predecessor the bolivar fuerte, are essentially worthless
...
## Cryptocurrencies
- There are 8,906 existing cryptocurrencies (that have value) <!-- change each semester -->
  - As per [investing.com](https://www.investing.com/crypto/currencies) (as of Aug 27, 2023)
...
![](images/cryptocurrency/btc-coin-symbol.svg)
:::
...
- The current blockchain (as of Aug 27, 2023) is  805,090 blocks (aka 'height'; [source](https://bitinfocharts.com/))
  - About 494 Gb in size ([source](https://www.blockchain.com/charts/blocks-size))
...
- The current target ($ct$) is  0000000000000000000561020000000 000000000000000000000000000000000
<!-- formula: ct=515199788182065911307653755120147792390991404454641664 -->
  - As of Aug 27, 2023 and can be found [here](http://learnmeabitcoin.com/glossary/target)
    - That site is almost 5,000 blocks behind...
  <!-- change each semester -->
  - The new target is harder than the old: specifically $ft/ct$ = 52,328,312,063,444 times harder (that's 52.3 *trillion*)
    - Actually it's about 55.6 trillion now<!-- set this value to diff, such as via the formula: diff=55.62*10^12 -->
...
- The current target is 55.6 trillion times harder
  - A random nonce now has a 1 in $2.39 \times 10^{23}$ chance of success
...
- Consider a 1 in $2.39 \times 10^{23}$ chance of success
- To generate $2.39 \times 10^{23}$ hashes at 800 million (8 Mhash) per second on a fast graphics card, this will take 9.5 *million years*
...
- This would cost about 3.7 billion dollars <!-- change each semester; formula: elecost=gpuyears*400/1000*24*365.25*0.1104 -->
...
## Reward per block
- The current reward (as of Aug 27, 2023) is 6.25 BTC
  <!-- change each semester -->
...
- At a price of about $26k per block ([source](https://www.coindesk.com/price/bitcoin)), 6.35 BTC is  about $166k for a properly signed block   <!-- change each semester --><!-- formula: reward=166*1000 -->
  - But if it costs almost 4 billion dollars in electricity, perhaps it's not all that worth it
...
## Hashes per second
From [blockchain.com](https://www.blockchain.com/charts/hash-rate), and for the entire Bitcoin network; E = exa = $10^{18}$
<!-- change each semester -->
<!-- formula: hashrate=390.9*10^18 -->
...
- Consider the [DragonX AntMiner S19](https://www.amazon.com//dp/B08ZNMGKZ1/), which costs `$`1,200 (as of Aug 27, 2023)
...
<!-- change each semester -->
- We know that:
  - The current hash rate is about 391 EH/s
  - Blocks take about 10 minutes to process (actually about 12m 6s right now ([source](https://bitinfocharts.com/)))
  - The [DragonX AntMiner S19](https://www.amazon.com/dp/B08ZNMGKZ1) claims to hash 90 TH/s <!-- formula: minerrate=90*10^12 -->
- 391 exa / 90 tera = $391 \cdot 10^{18} / 90 \cdot 10^{12} = 4.34 \cdot 10^{6}$ or 4 million <!-- formula: blockreward=hashrate/minerrate -->
- Thus, you would expect to get one 4-millionth of a block's reward in a pooled setup <!-- formula: profitperblock=reward/blockreward -->
- At `$`166 per block, that's about 3.8 *cents* per block
...
<!-- change each semester -->
- At `$`166 per block, that's about 3.8 cents per block
  - Which is $5.50 per day <!-- formula: profitperday=profitperblock*24*6 -->
    - This assumes 10 minute blocks; it's a less with 12m6s blocks
  - You will make back your initial investment of `$`1,200 (plus tax) 220 days
    - That's 7.3 months
- This assumes that the numbers don't change!
  - And that you aren't paying for electricity
  - And we know the reward per block will go down in April of 2024...
...
## [DragonX AntMiner S19](https://www.amazon.com/dp/B08ZNMGKZ1) analysis
<!-- change each semester -->
- You will make $5.50 in BTC per day
...
- Stated power usage: 3,250 W or 3.25 kW
  ...
  - At Charlottesville's rate of 11.04&cent; per kWh, that's `$`8.61 per day
    - About `$`260 per month
- So your total daily profit is: `$`5.50 - `$`8.61 or
  - *Negative* `$`3.11 cents per day!
...
- When done on a large scale
  - One can make deals with power companies, locate where power is cheaper, etc.
