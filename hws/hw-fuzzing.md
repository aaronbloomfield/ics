...
In this assignment, you will be creating a *network fuzzer*, which is a program that will try out many different URLs to see if any exist on a web server.  Given a base URL, such as `http://server.com/FUZZ`, and a word list containing many items, it will replace the string `FUZZ` with each of the words in the word list, and see if that URL exists.
...
This assignment must be done in a recent version of Python.  You will want to create a virtual environment in Python: `virtualenv fuzzer`.  You then enter that virtual environment; on Linux it is `source fuzzer/bin/activate`; on Windows it's `...`.  Install all the packages listed in the [requirements.txt](fuzzing/requirements.txt) file: `pip install -r fuzzing/requirements.txt`
...
When you are ready to test this, read the 'Testing' section, below, for easy ways to test locally on your computer.


#### Output

The output for this assignment is very specific.  At this point, any URL that does *NOT* return a 404 should display the output in the following EXACT format.  (Note that you will modify what URLs to print later).
...
This is the result of calling the fuzzer as: `python3 fuzzer.py -u http://ffuf.me/cd/basic/FUZZ -mc 200 -w common.txt`

Note that the output is the status number, a single space, and the URL.  The order you print it out does not matter.

This call to your code, with a reduced word list that contains those two values (`class` and `development.log`) will be the visible test when you submit your assignment.  Other hidden tests will be used to grade your assignment.
...
First test it locally, then test it remotely.

#### Local testing

Initially, as you are developing your code, you will want to test it out locally.  One of the Python packages installed is uvicorn, which is a small Python-based web server.  To run it, you first need to save the [server.py](fuzzing/server.py.html) ([src](fuzzing/server.py)) file.  This file has a `urls` list that is the set of URLs that it will respond with a 200 (OK) to; everything else it responds with a 404 (not found).  

You are welcome to change the elements in that list during testing -- just note that each one has to start with a forward slash.  If you are running uvicorn, and you edit the server.py file, it should detect this and reload the contents.

You will then run your uvicorn server as follows:
...
Following the output specifications described above, the output should be exactly as follows.  Note that the order does not matter, as long as the format of each line is exact.
...
You should only submit your fuzzer.py file to Gradescope.  The only visible test will be the `python3 fuzzer.py http://ffuf.me/cd/basic/FUZZ -w common.txt` with a reduced word list (that will still contain `class` and `development.log`).
